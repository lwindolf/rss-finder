...
export class Subscriber {
    ...
    autodiscover(el, data, baseUrl) {
            ...
            {{#compare results.length ">" 0}}
                ...
                {{#each results}}
                    <div class='result block'>
                        <div class='resultDetails'>{{this}}</div>                        
                    </div>
                {{/each}}
            {{/compare}}
        `, {
        ...
        el.getRootNode().querySelectorAll('.resultDetails').forEach((result) => {
            result.addEventListener('click', (event) => {
                event.preventDefault();
                this.preview(event.target.innerText, event.target.closest('.result'));
            });
        });
    }
